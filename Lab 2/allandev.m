function av = allandev(pdata,pname,fig,color,frequency)
   
   data.type = pname; % need to be a string 
   data.freq = pdata; 
   data.time = 1:1/frequency:length(pdata);
   data.rate = frequency;     % sampling at 200Hz 
   data.units = 's';   
   taumax = floor(length(pdata)/4); 
   tau = 1:taumax; 
    
   [av, s, errorb, tau2] = allan2(data,tau,data.type,1,fig,color);


function [retval, s, errorb, tau] = allan2(data,tau,name,verbose,fig,color)
...
elseif isfield(data,'rate') && data.rate ~= 0;
    ...
    error('Either "time" or "rate" must be present in DATA.  "help allan" for details. [err1]');
end
s.std=std(data.freq);
...
if verbose >= 1 % show ADEV results
    ...
    if ~isempty(sm)
        ...
        hold on; plot([tau; tau],[sm+sme; sm-sme],strcat('-',color),'LineWidth',max(plotlinewidth-1,2));
